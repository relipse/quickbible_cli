...
class QuickBibleCli
{
    ...
    public function getHistoryLast(): string|false
    {
    ...
    public function addToHistory(string $ref)
    {
        $this->history[] = $ref;
    }

    public function saveHistory(): false|int
    {
        return file_put_contents(__DIR__ . '/.qb-history.json', json_encode($this->history));
    }

    public function loadHistory(): array
    {
        $his = [];
        if (file_exists(__DIR__ . '/.qb-history.json')) {
            $contents = file_get_contents(__DIR__ . '/.qb-history.json');
            if (!empty($contents)) {
                $his = json_decode($contents, true);
                if (!is_array($his)) {
                    $his = [];
                }
            }
    ...
    public function getLastOptions()
    {
        ...
        $this->last_options = $this->options = json_decode(file_get_contents($this->cfg_file), true);
        return true;
    }
    ...
    public function parseCli()
    {
        ...
        $shortopts .= "o"; //old testament
        $shortopts .= "n"; //new testament
        $shortopts .= 'h';  //help
        ...
        if (isset($options['n']) || isset($options['nt'])){
            $this->options['nt'] = true;
            $this->options['ot'] = false;
        }else if (isset($options['o']) || isset($options['ot'])){
            $this->options['nt'] = false;
            $this->options['ot'] = true;
        }
        if (isset($options['context'])) {
            $this->options = $this->last_options;
            $this->options['context'] = true;
        }
        return true;
    }
    ...
    {
        if (!empty($this->options['nt'])){
            $b = rand(40, 66);
        }else if (!empty($this->options['ot'])){
            $b = rand(1, 39);
        }else {
            $b = rand(1, 66);
        }
    ...
    public function referenceLookup($dbh, $regs, int|false $context, array & $ref)
    {
        $ref = [];
        $bk = $regs[1];
        $ch = $regs[2];
        ...
        if (isset($last_verse)) {
            $vs_max = $last_verse;
            $ref['chapter'] = (int)$last_chapter;
            $ref['vs_min'] = (int)$first_verse;
            $ref['vs_max'] = (int)$vs_max;
            $ref['q'] = ucwords(strtolower($ref['book'])).' '.$ref['chapter'];
            if (is_numeric($ref['q'][0])){
                $ref['q'][1] = strtoupper($ref['q'][1]);
            }
            $ref['q'] .= ':'.$ref['vs_min'];
            if ($ref['vs_max'] > $ref['vs_min']) {
                $ref['q'] .= '-'.$ref['vs_max'];
            }
            return true;
        }else{
    ...
    public function determinePath()
    {
        ...
        if (!isset($this->options['context']) &&  isset($this->options['rv'])) {
            $this->getRandomVerse($dbh);

            echo $this->options['q'];
            $opts = $this->options;
            unset($opts['rv']);
            $this->putOptions($opts);
            if (!isset($this->options['nnl'])) {
                echo "\n";
            } else {
                echo " ";
        ...
        if (!empty($this->options['q'])) {
            if (preg_match('/([\da-z][a-z][a-z])[a-z]* (\d+):?(\d+)?\-?(\d+)?/i', $this->options['q'], $regs)) {
                $context = isset($this->options['context']) ? 3 : false;
                $ref = [];
                $this->referenceLookup($dbh, $regs, $context, $ref);
                if (isset($ref['q'])) {
                    echo "\n   -".$ref['q'];
                    $this->options['q'] = $ref['q'];
                    $this->putOptions($this->options);
                }
            } else {
                $this->search($dbh);
            }
    ...
    public static function getBookNumber(string $bk): int|false
    {
        $bk = strtoupper($bk);
